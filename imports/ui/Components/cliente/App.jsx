              ...
              this.props.tvsimages.map(image => (
                image.fileFormat.split('/')[0] === 'image' ? (
                  <div key={image._id} data-cycle-timeout={image.time}>
                    <img src={`http://localhost:3000/cfs/files/Images/${image.imagecode}?store=container`}></img>
                  </div>
                ) : (
                  <div key={image._id} data-cycle-timeout="200">
                    <video id="myVideo">
                      <source src={`http://localhost:3000/cfs/files/Images/${image.imagecode}?store=container`} type={image.fileFormat}/>
                    </video>
                  </div>
                )
              ))
